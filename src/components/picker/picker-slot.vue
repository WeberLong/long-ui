<template>
  <div class="ui-picker-slot" :class="{'ui-picker-slot-divider': divider}" :style="{'width': width}">
    <div v-if="!divider" class="ui-picker-slot-wrapper" :class="{'animate': animate}" :style="{'height': contentHeight + 'px'}" ref="wrapper">
      <div class="ui-picker-item" :style="{'text-align': textAlign}" :class="{'selected': item === value}"
    ...
    initEvents () {
      ...
      drag.start(() => {
        ...
        Vue.nextTick(() => {
          ...
          this.$emit('slot-select', this.translate2Value(translate))
        })
      })
    }
  },
  watch: {
    values (newVal) {
      if (this.valueIndex === -1) {
        this.value = (newVal || [])[0]
        // this.$emit('slot-select', (newVal || [])[0])
      }
    },
    value () {
      this.doOnValueChange()
...
.ui-picker-slot-wrapper.animate{
